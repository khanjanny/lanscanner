...
then
	...
	for line in $(cat .servicios/rpc.txt); do
		...
		port=`echo $line | cut -f2 -d":"`	
		echo -e "[+] Escaneando $ip:$port"		
		echo "nmap -n -p $port $ip --script=nfs-ls.nse" > logs/vulnerabilidades/"$ip"_"$port"_nfs.txt 2>/dev/null 
		...
		echo "nmap -n -p $port $ip --script=rpcinfo" > logs/enumeracion/"$ip"_"$port"_info.txt 2>/dev/null 
		nmap -Pn -n -p $port $ip --script=rpcinfo >> logs/enumeracion/"$ip"_"$port"_info.txt 2>/dev/null 
...
then	
	...
	for line in $(cat .servicios/redis.txt); do
		...
		port=`echo $line | cut -f2 -d":"`	
		echo -e "[+] Escaneando $ip:$port"			
		echo "nmap -n -p $port $ip --script redis-info" > logs/enumeracion/"$ip"_"$port"_redis.txt 2>/dev/null
		nmap -Pn -n -p $port $ip --script redis-info >> logs/enumeracion/"$ip"_"$port"_redis.txt 2>/dev/null
		grep "|" logs/enumeracion/"$ip"_"$port"_redis.txt | egrep -iv "ACCESS_DENIED|false|Could|ERROR"  > .enumeracion/"$ip"_"$port"_redis.txt						
	done
...
then	
	...
	for line in $(cat .servicios/rmi.txt); do
		...
		port=`echo $line | cut -f2 -d":"`
		echo -e "[+] Escaneando $ip:$port"
		echo "nmap -n -p $port $ip --script rmi-vuln-classloader" > logs/vulnerabilidades/"$ip"_"$port"_vuln.txt 2>/dev/null
		nmap -Pn -n -p $port $ip --script rmi-vuln-classloader >> logs/vulnerabilidades/"$ip"_"$port"_vuln.txt 2>/dev/null
		grep "|" logs/vulnerabilidades/"$ip"_"$port"_vuln.txt  | egrep -iv "ACCESS_DENIED|false|Could|ERROR" > .vulnerabilidades/"$ip"_"$port"_vuln.txt
...
then
	...
	do     				
		...
		echo "nmap -n -p $port --script http-vuln-cve2017-5689 $ip" > logs/vulnerabilidades/"$ip"_"$port"_intelBypass.txt 2>/dev/null
		nmap -n -p $port --script http-vuln-cve2017-5689 $ip >> logs/vulnerabilidades/"$ip"_"$port"_intelBypass.txt 2>/dev/null
		grep "|" logs/vulnerabilidades/"$ip"_"$port"_intelBypass.txt | egrep -iv "ACCESS_DENIED|false|Could|ERROR" > .vulnerabilidades/"$ip"_"$port"_intelBypass.txt
			...
			#echo "FILE $prefijo$FILE"			
				...
				if grep -q ";" "$prefijo$FILE" 2>/dev/null; then
					...
					for subdominio in $lista_subdominios; do													
						...
						noRedireccion=$?	
						echo -e "\t\tnoEscaneado $noEscaneado noRedireccion $noRedireccion"
						# 1= no coincide (no redirecciona a otro dominio o es error de proxy)						
						if [[ $noRedireccion -eq 1 &&  $noEscaneado -eq 1 ]];then # El sitio no fue escaneado antes y no redirecciona a otro dominio o es error de proxy
...
#								egrep --color=never "^200" logs/vulnerabilidades/"$subdominio"_"$port"_archivosDefecto.txt  | awk '{print $2}' >> .vulnerabilidades/"$subdominio"_"$port"_archivosDefecto.txt  
							...
							if [[ $greprc -eq 0 ]];then 		
								...
								wpscan --random-agent --url http://$subdominio/ --enumerate u --follow-redirection > .enumeracion/"$subdominio"_"$port"_wpscan.txt
				...
				#prefijoDominio="${subdominio/$DOMAIN/}" # mail.  www. 
				#sed -i "s/$prefijoDominio//g" webClone/https-$subdominio.html # borrar mail.  www.
				sed -i "s/www.//g" webClone/http-$ip.html
				...
				if [[ $noRedireccion -eq 1 &&  $noEscaneado -eq 1 ]];then # El sitio no fue escaneado antes y no redirecciona a otro dominio o es error de proxy												
					...
					if [[ $greprc -eq 0 ]];then 		
						...
						wpscan  --update >/dev/null
						wpscan --random-agent --url http://$ip/ --enumerate u --follow-redirection > .enumeracion/"$ip"_"$port"_wpscan.txt
					...
					if [[ $greprc -eq 0 && ! -f .enumeracion/"$ip"_"$port"_webarchivos.txt  ]];then # si el banner es IIS y no se enumero antes
						...
						echo "nmap -p $port --script http-vuln-cve2015-1635.nse $ip" >> logs/vulnerabilidades/"$ip"_"$port"_ms15034.txt
						nmap -p $port --script http-vuln-cve2015-1635.nse $ip >> logs/vulnerabilidades/"$ip"_"$port"_ms15034.txt
		...
		while true; do
				...
				if grep -q ";" "$prefijo$FILE" 2>/dev/null; then
					...
					for subdominio in $lista_subdominios; do
						...
						sed -i "s/https/http/g" webClone/https-$subdominio.html
						prefijoDominio="${subdominio/$DOMAIN/}" # mail.  www. 
						sed -i "s/$prefijoDominio//g" webClone/https-$subdominio.html 2>/dev/null # borrar mail.  www.
						sed -i "s/www.//g" webClone/https-$subdominio.html
						...
						egrep -qi "301 Moved|302 Found|500 Proxy Error|400 Bad Request" .enumeracion/"$subdominio"_"$port"_webData.txt
						noRedireccion=$?	
						echo -e "\t\tnoEscaneado $noEscaneado noRedireccion $noRedireccion"
						# 1= no coincide (no redirecciona a otro dominio o es error de proxy)						
						if [[ $noRedireccion -eq 1 &&  $noEscaneado -eq 1 ]];then # El sitio no fue escaneado antes y no redirecciona a otro dominio o es error de proxy
							...
							if [[ $greprc -eq 0 ]];then 		
								...
								wpscan --random-agent  --url https://$subdominio/ --enumerate u --follow-redirection > .enumeracion/"$subdominio"_"$port"_wpscan.txt 
								egrep --color=never "\| [0-9]{1}" .enumeracion/"$subdominio"_"$port"_wpscan.txt | grep -v plugin | awk '{print $4}' > .vulnerabilidades/"$subdominio"_"$port"_wpusers.txt
							fi
							###########################	
				...
				sed -i "s/\/index.php//g" webClone/https-$ip.html
				sed -i "s/https/http/g" webClone/https-$ip.html
				#prefijoDominio="${subdominio/$DOMAIN/}" # mail.  www. 
				#sed -i "s/$prefijoDominio//g" webClone/https-$subdominio.html # borrar mail.  www.
				sed -i "s/www.//g" webClone/https-$ip.html
				checksumline=`md5sum webClone/https-$ip.html` 							
				md5=`echo $checksumline | awk {'print $1'}` 													
				egrep -iq $md5 webClone/checksumsEscaneados.txt
				noEscaneado=$?						
				
				egrep -qi "301 Moved|302 Found|500 Proxy Error|400 Bad Request" .enumeracion/"$ip"_"$port"_webData.txt
				noRedireccion=$?	
				# 1= no coincide (no redirecciona a otro dominio o es error de proxy)						
...
then
	...
	do     						
		...
		echo -e "\t[+]Probando vulnerabilidad cve2014-2128 \n $RESET"
...
then
	...
	do     						
		...
		if [[ $greprc -eq 0 ]] ; then			
			...
			medusa -h $ip -u admin -p admin -M telnet >> logs/vulnerabilidades/"$ip"_23_cisco.txt 2>/dev/null
			grep --color=never SUCCESS logs/vulnerabilidades/"$ip"_23_cisco.txt > .vulnerabilidades/"$ip"_23_cisco.txt 2>/dev/null
		fi		
		...
		if [[ $greprc -eq 0 ]] ; then			
			echo -e "\t Probando SSH \n $RESET"
			medusa -h $ip -u admin -p admin -M telnet >> logs/vulnerabilidades/"$ip"_22_cisco.txt 2>/dev/null			
			grep --color=never SUCCESS logs/vulnerabilidades/"$ip"_22_cisco.txt > .vulnerabilidades/"$ip"_22_cisco.txt 2>/dev/null
		fi								
		echo ""													 		
	...
	while true; do
		...
		then
			if [[ ${line} != *"Listado directorio"* &&  ${line} != *"wp-"*  ]] ; then
				...
				if [[ (${path} == *"usuarios"* || ${path} == *"Login"*  || ${path} == *"Login"* ) && (${internet} == "s" )]];then 
...
sort .servicios/admin-web2.txt | uniq > .servicios/admin-web.txt 2>/dev/null
